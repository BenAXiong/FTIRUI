...
export function initWorkspaceCanvas() {
  ...
  function patchLayout(entry, patchObj) {
    ...
    // 2) Update our in-memory state so session restore & undo/redo stay correct
    entry.state = entry.state || {};
    entry.state.figure = entry.state.figure || {};
    const layout = (entry.state.figure.layout = entry.state.figure.layout || {});

    // Support both nested keys (e.g., { xaxis:{ showgrid:true } })
    // and dotted relayout keys (e.g., { 'xaxis.showgrid': true }).
  ...
  const axisExists = (entry, axisKey) => {
    if (isPrimaryAxis(axisKey)) return true;
    if (!entry) return false;
    const figure = entry.state?.figure;
  ...
  const forEachAxis = (entry, axes, cb) => {
    const layout = entry?.state?.figure?.layout || {};
    axes.forEach((axis) => {
      if (!axisExists(entry, axis)) return;
  ...
  function handleHeaderAction(entry, act, payload = {}) {
    switch (act) {
      ...
      case 'axes-thickness': {
        ...
        const L = entry.state?.figure?.layout || {};
        const xColor = L?.xaxis?.linecolor || '#444';
        const yColor = L?.yaxis?.linecolor || '#444';

        // Only touch linewidth/gridwidth/linecolor using dotted keys — do NOT send xaxis:{...}
        patchLayout(entry, {
          'xaxis.linewidth': w,
          'yaxis.linewidth': w,
      ...
      case 'axes-side': {
        const s = payload || {};
        const L  = entry.state?.figure?.layout || {};
        const wx = Math.max(1, Number(L?.xaxis?.linewidth) || 2);
        const wy = Math.max(1, Number(L?.yaxis?.linewidth) || 2);
        const cx = L?.xaxis?.linecolor || '#444';
        const cy = L?.yaxis?.linecolor || '#444';
      ...
      case 'grid-minor': {
        ...
        const L = entry.state?.figure?.layout || {};
        const patch = {};
        forEachAxis(entry, ['xaxis', 'yaxis', 'xaxis2', 'yaxis2'], (axis, axisState) => {
      ...
      case 'grid-minor-subdiv': {
        ...
        forEachAxis(entry, ['xaxis', 'yaxis', 'xaxis2', 'yaxis2'], setMinorDtick);
      ...
      case 'ticks-placement': {
        ...
        const L = entry.state?.figure?.layout || {};
        const hasX2 = axisExists(entry, 'xaxis2');
        const hasY2 = axisExists(entry, 'yaxis2');
        const patch = {
          'xaxis.ticks': p,
          'yaxis.ticks': p
        };
        ...
        preserveAxisDecorations(L, 'xaxis', patch);
        preserveAxisDecorations(L, 'yaxis', patch);
        if (hasX2) preserveAxisDecorations(L, 'xaxis2', patch);
        if (hasY2) preserveAxisDecorations(L, 'yaxis2', patch);
      ...
      case 'ticks-labels': {
        ...
        if (axisExists(entry, 'xaxis2')) patch['xaxis2.showticklabels'] = on;
        if (axisExists(entry, 'yaxis2')) patch['yaxis2.showticklabels'] = on;
        patchLayout(entry, patch);
        break;
      }
      ...
      case 'ticks-major-dtick': {
        // null → auto; number → fixed spacing
        const L = entry.state?.figure?.layout || {};
        const hasX2 = axisExists(entry, 'xaxis2');
        const hasY2 = axisExists(entry, 'yaxis2');
        const patch = {};
      ...
      case 'ticks-minor-subdiv': {
        ...
        setMinor('xaxis');
        setMinor('yaxis');
        if (axisExists(entry, 'xaxis2')) setMinor('xaxis2');
        if (axisExists(entry, 'yaxis2')) setMinor('yaxis2');
      ...
      case 'smooth': {
        ...
        data.forEach((tr) => {
          tr.line = tr.line || {};
          tr.line.shape = on ? 'spline' : 'linear';
          if (on) tr.line.smoothing = 1.15; else delete tr.line.smoothing;
        });
        renderPlot(entry); // your renderPlot does Plotly.react
  ...
  const registerPanel = (incomingState, {
    ...
    axesPopover.onOpen = () => {
      ...
      // Thickness pills — infer from linewidth
    ...
    const handleAxesOutsideClick = (event) => {
      ...
      const currentLayout = entry.state?.figure?.layout || {};
      ...
      <div class="workspace-panel-popover-section">
        ...
        <div class="workspace-panel-popover-items" data-role="ticks-major">
          ...
          <div class="ms-3 d-flex align-items-center gap-2" data-role="ticks-major-offset">
            ...
            <input type="number" step="any" class="form-control form-control-sm" style="width:90px" placeholder="X₀">
            <input type="number" step="any" class="form-control form-control-sm" style="width:90px" placeholder="Y₀">
          ...
          <div class="ms-3 d-flex align-items-center gap-2" data-role="ticks-major-dtick">
            ...
            <input type="number" step="any" class="form-control form-control-sm" style="width:90px" placeholder="ΔX">
            <input type="number" step="any" class="form-control form-control-sm" style="width:90px" placeholder="ΔY">
    ...
    ticksPopover.onOpen = () => {
      ...
      // Major placement (assume both axes share the same, pick X’s as source of truth)
      ...
      if (offWrap) {
        const xInput = offWrap.querySelector('input[placeholder="X₀"]');
        const yInput = offWrap.querySelector('input[placeholder="Y₀"]');
      ...
      if (dtWrap) {
        ...
        dtWrap.querySelector('input[placeholder="ΔX"]').value = Number.isFinite(dx) ? String(dx) : '';
        dtWrap.querySelector('input[placeholder="ΔY"]').value = Number.isFinite(dy) ? String(dy) : '';
      }

      const mplace = (X.minor?.ticks ?? '');
      ticksPopover.querySelectorAll('[data-role="ticks-minor"] [data-minor-placement]')
        .forEach(b => {
          const val = b.dataset.minorPlacement;   // ✅ correct
    ...
    ticksPopover.addEventListener('click', (e) => {
      ...
      //   const x0raw = wrap.querySelector('input[placeholder="X₀"]').value;
      //   const y0raw = wrap.querySelector('input[placeholder="Y₀"]').value;
      ...
      //   const dxRaw = wrap.querySelector('input[placeholder="ΔX"]').value;
      //   const dyRaw = wrap.querySelector('input[placeholder="ΔY"]').value;
